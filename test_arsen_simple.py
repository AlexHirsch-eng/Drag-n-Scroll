#!/usr/bin/env python
"""Comprehensive test script for arsen user functionality (no emoji)"""
import requests
import json
...
def test_arsen_user():
    ...
    if users_response.status_code == 200:
        ...
        for user in users:
            if user.get('username') == 'arsen':
                arsen_id = user.get('id')
                break

    if not arsen_id:
        print(f"[INFO] arsen not in suggested, checking all users...")
        # Try to find arsen by checking existing users
    ...
    if not arsen_id:
        print(f"[FAIL] Could not find arsen user")
        return False

    print(f"[OK] Found arsen user: ID={arsen_id}")
    ...
    if create_chat_response.status_code not in [200, 201]:
        print(f"[FAIL] Failed to create direct chat: {create_chat_response.status_code}")
        print(create_chat_response.text)
        return False

    chat_room = create_chat_response.json()
    room_id = chat_room.get('id')
    print(f"[OK] Direct chat created: Room ID={room_id}")
    other_user = chat_room.get('other_user', {})
    print(f"      Other user: {other_user.get('username', 'N/A')}")
    ...
    send_msg_response = requests.post(
        ...
        json={
            "room": room_id,
            "text": "Privet arsen! Eto testovoe soobshchenie.",
            "message_type": "text"
        }
    )

    if send_msg_response.status_code != 201:
        print(f"[FAIL] Failed to send message: {send_msg_response.status_code}")
    ...
    if get_msgs_response.status_code != 200:
        print(f"[FAIL] Failed to get messages: {get_msgs_response.status_code}")
    ...
    if rooms_response.status_code != 200:
        print(f"[FAIL] Failed to get chat rooms: {rooms_response.status_code}")
        print(rooms_response.text)
        return False

    rooms = rooms_response.json()
    print(f"[OK] Retrieved {len(rooms)} chat rooms")
    ...
    if arsen_login_response.status_code != 200:
        print(f"[FAIL] Failed to login as arsen: {arsen_login_response.status_code}")
        print(arsen_login_response.text)
        return False

    arsen_tokens = arsen_login_response.json()
    arsen_access = arsen_tokens.get('access')
    print(f"[OK] arsen logged in successfully")
    ...
    if arsen_rooms_response.status_code != 200:
        print(f"[FAIL] Failed to get arsen's chat rooms: {arsen_rooms_response.status_code}")
        print(arsen_rooms_response.text)
        return False

    arsen_rooms = arsen_rooms_response.json()
    print(f"[OK] arsen can see {len(arsen_rooms)} chat rooms")
    ...
    for room in arsen_rooms:
        ...
        if other_user.get('username') == 'testuser':
            found_chat = True
            print(f"[OK] arsen can see chat with testuser")
            preview = room.get('last_message_preview', 'No messages')[:30]
            print(f"      Last message: {preview}...")
            break

    if not found_chat:
        print(f"[WARN] arsen cannot see chat with testuser")
    ...
    print("\n[COMPLETE] arsen user is fully functional!")
